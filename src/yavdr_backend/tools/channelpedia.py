...
async def get_categories():
    ...
    channel_categories: list[dict[str, Any]] = []
    async with aiohttp.ClientSession() as session:
        async with session.get(
            "http://channelpedia.yavdr.com/restful/sources", raise_for_status=True
        ) as resp:
            ...
            for source, position_dict in data.items():
                ...
                for _position, groups in position_dict.items():
                    if not groups:
                        groups = ["all"]
...
async def get_channels() -> defaultdict[
    str, defaultdict[str, defaultdict[str, list[Subgroup]]]
]:
    channel_data: defaultdict[
        str, defaultdict[str, defaultdict[str, list[Subgroup]]]
    ] = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
    # for source, position_dict in get_categories().items():
    for source, position_dict in {
        "DVB-C": {"de_KabelDeutschland_Muenchen": ["de", "sky_de"]}
    }.items():
        for position, groups in position_dict.items():
            if not groups:
                groups = ["all"]
            for group in groups:
                channel_data[source][position][group] += await get_channel_group(
                    source=source, position=position, group=group
                )
    return channel_data
...
async def get_channel_group(source, position, group) -> list[Subgroup]:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as resp:
            data: list[ChannelpediaSubgroup] = [
                ChannelpediaSubgroup(**subgroup)
                for subgroup in (await resp.json()).get("result", [])
            ]
