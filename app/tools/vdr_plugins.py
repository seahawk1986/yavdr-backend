...
def parse_config_file(path: Path) -> Mapping[str, List[str]]:
    ...
    for line in path.read_text().splitlines():
        line = line.strip()
        if not on_plugin_config and (not line or line.startswith('#')):  # skip comments before the actual config
            continue
...
def extract_name_prio(plugin_links: List[Path]) -> Iterator[Tuple[str, int]]:
...
def read_plugin_help(plugin_name: str=None) -> Mapping[str, List[str]]:
    args = ["vdr"]
    if plugin_name is not None:
        args.append(f"-P{plugin_name}")
    args.append("-h")
    _, _, help_text = subprocess.run(args, capture_output=True, text=True).stdout.partition('Plugins: vdr -P"name [OPTIONS]"\n\n')
...
def read_plugins() -> List[PluginConfig]:
    ...
    static_configuration_files = set(p for p in active_configuration_files if not p.is_symlink())
    linked_configuration_files = active_configuration_files - static_configuration_files
    disabled_configuration_files = available_configuration_files - linked_configuration_files

    configuration_files = {}
    cfg_priorities = {name: priority for name, priority in extract_name_prio(linked_configuration_files)}
    ...
    enabled_plugins_symlinks = [
        p for p in VDR_ARGS_DIR.glob('*.conf')
        if p.is_symlink() and p.resolve().parent == VDR_CONFIG_DIR]
    priorities = {name: priority for name, priority in extract_name_prio(enabled_plugins_symlinks)}
