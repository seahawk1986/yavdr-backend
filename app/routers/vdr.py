...
from app.tools.channel_interfaces import Channel
...
from app.interfaces.vdr_channels import DeTvdrVdrChannelInterface
from app.interfaces.vdr_epg import DeTvdrVdrEpgInterface
from app.interfaces.vdr_plugins import DeTvdrVdrPluginmanagerInterface
from app.interfaces.vdr_recordings import DeTvdrVdrRecordingInterface
from app.interfaces.vdr_timers import DeTvdrVdrTimerInterface
from app.interfaces.vdr_setup import DeTvdrVdrSetupInterface
from app.interfaces.vdr_skin import DeTvdrVdrSkinInterface
from app.interfaces.vdr_status import signal_generator as vdr_status_event_generator
from app.interfaces.system_backend import (
...
async def getSVDRP_Recording_ID_by_name(title):
    ...
    async for line in async_send_svdrpcommand("lstr"):
        code, data = line.strip().split("-", maxsplit=1)
...
async def write_args_config(
    config: PluginConfig, User=Depends(get_current_active_user)
):
...
def save_vdr_channels(channel_list: list[str]):
    ...
    config_dir = Path(
        os.environ.get(
            "VDR_CONFIG_DIR",
            pkgconfig.variables("vdr").get("configdir", "/var/lib/vdr"),
        )
    )
    print(f"{config_dir=}")
...
async def get_vdr_channels_with_groups(
    ...
    async for line in async_send_svdrpcommand("lstc :ids :groups"):
        code, r = line.strip().split("-", maxsplit=1)
        # print(r)
...
async def delete_channel(channel_id: str, User=Depends(get_current_active_user)):
...
async def get_audiochannels() -> list[AudioChannel]:
    ...
    async for line in async_send_svdrpcommand("AUDI"):
        code, line = line.rstrip().split("-", maxsplit=1)
        try:
...
async def set_audiochannel(channel: int):
    r = []
    async for line in async_send_svdrpcommand("AUDI"):
        code, line = line.strip().split("-", maxsplit=1)
